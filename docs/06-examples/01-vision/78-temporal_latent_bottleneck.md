...
**저자:** [Aritra Roy Gosthipaty](https://twitter.com/ariG23498), [Suvaditya Mukherjee](https://twitter.com/halcyonrayes)  
**생성일:** 2023/03/12  
**최종편집일:** 2023/03/12  
**설명:** Image Classification with Temporal Latent Bottleneck Networks.

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/vision/ipynb/temporal_latent_bottleneck.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/vision/temporal_latent_bottleneck.py){: .btn .btn-blue }
...
_Note_: This example makes use of `TensorFlow 2.12.0`, which must be installed into our system
...
Load dataset into [`tf.data.Dataset`](https://www.tensorflow.org/api_docs/python/tf/data/Dataset) object
--------------------------------------------------------------------------------------------------------

*   We take the `np.ndarray` instance of the datasets and move them into a [`tf.data.Dataset`](https://www.tensorflow.org/api_docs/python/tf/data/Dataset) instance
...
This custom [`keras.layers.Layer`](/api/layers/base_layer#layer-class) is useful for generating patches from the image and transform them into a higher-dimensional embedding space using [`keras.layers.Embedding`](/api/layers/core_layers/embedding#embedding-class). The patching operation is done using a [`keras.layers.Conv2D`](/api/layers/convolution_layers/convolution2d#conv2d-class) instance instead of a traditional [`tf.image.extract_patches`](https://www.tensorflow.org/api_docs/python/tf/image/extract_patches) to allow for vectorization.
...
This custom [`keras.layers.Layer`](/api/layers/base_layer#layer-class) instance allows us to define a generic FFN along with a dropout.
...
This custom [`keras.layers.Layer`](/api/layers/base_layer#layer-class) instance is a `super`/`base` class that wraps a [`keras.layers.MultiHeadAttention`](/api/layers/attention_layers/multi_head_attention#multiheadattention-class) layer along with some other components. This gives us basic common denominator functionality for all the Attention layers/modules in our model.
...
This custom cell, implemented as a [`keras.layers.Layer`](/api/layers/base_layer#layer-class), is the integral part of the logic for the model. The cell's functionality can be divided into 2 parts: - **Slow Stream (Temporal Latent Bottleneck):**
...
We use the `AdamW` optimizer since it has been shown to perform very well on several benchmark tasks from an optimization perspective. It is a version of the [`keras.optimizers.Adam`](/api/optimizers/adam#adam-class) optimizer, along with Weight Decay in place.

For a loss function, we make use of the [`keras.losses.SparseCategoricalCrossentropy`](/api/losses/probabilistic_losses#sparsecategoricalcrossentropy-class) function that makes use of simple Cross-entropy between prediction and actual logits. We also calculate accuracy on our data as a sanity-check.
...
Thanks to [Aniket Didolkar](https://www.aniketdidolkar.in/) (the first author) and [Anirudh Goyal](https://anirudh9119.github.io/) (the third author) for revieweing our work.
