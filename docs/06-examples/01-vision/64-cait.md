...
**생성일:** 2022/09/19  
**최종편집일:** 2022/11/21  
**설명:** Implementing an image transformer equipped with Class Attention and LayerScale.

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/vision/ipynb/cait.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/vision/cait.py){: .btn .btn-blue }
...
The readers are assumed to be familiar with Vision Transformers already. Here is an implementation of Vision Transformers in Keras: [Image classification with Vision Transformer](https://keras.io/examples/vision/image_classification_with_vision_transformer/).
...
$$
x_l' = x_l + \text{diag}(\lambda_{l, 1}, ...,\lambda_{l, d})) \times \text{SA}(\eta(x_l)) \\
x_{l+1} = x'_l + \text{diag}(\lambda'_{l, 1}, ...,\lambda'_{l, d})) \times \text{FFN}(\eta(x'_l))
$$
...
*   `LayerScaleBlockClassAttention()` which returns a [`keras.Model`](/api/models/model#model-class). It is a Transformer block equipped with Class Attention, LayerScale, and Stochastic Depth. It operates on the CLS embeddings and the image patch embeddings.
