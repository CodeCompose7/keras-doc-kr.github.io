...
**생성일:** 2023/01/08  
**최종편집일:** 2023/01/08  
**설명:** Training a Dense-layer model using the Forward-Forward algorithm.

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/vision/ipynb/forwardforward.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/vision/forwardforward.py){: .btn .btn-blue }
...
As this example requires the customization of certain core functions with [`keras.layers.Layer`](/api/layers/base_layer#layer-class) and `keras.models.Model`, refer to the following resources for a primer on how to do so:

*   [Customizing what happens in `model.fit()`](https://keras.io/guides/customizing_what_happens_in_fit)
...
In this custom layer, we have a base [`keras.layers.Dense`](/api/layers/core_layers/dense#dense-class) object which acts as the base `Dense` layer within. Since weight updates will happen within the layer itself, we add an [`keras.optimizers.Optimizer`](/api/optimizers#optimizer-class) object that is accepted from the user. Here, we use `Adam` as our optimizer with a rather higher learning rate of `0.03`.
