...
**저자:** [Aritra Roy Gosthipaty](https://twitter.com/arig23498), [Ritwik Raha](https://twitter.com/ritwik_raha)  
**생성일:** 2021/11/08  
**최종편집일:** 2021/11/08  
**설명:** Neural Style Transfer with Adaptive Instance Normalization.

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/generative/ipynb/adain.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/generative/adain.py){: .btn .btn-blue }
...
where `sigma` is the standard deviation and `mu` is the mean for the concerned variable. In the above equation the mean and variance of the content feature map `fc` is aligned with the mean and variance of the style feature maps `fs`.
...
This is the trainer module. We wrap the encoder and decoder inside a [`tf.keras.Model`](/api/models/model#model-class) subclass. This allows us to customize what happens in the `model.fit()` loop.
