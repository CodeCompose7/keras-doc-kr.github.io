---
layout: default
title: 텍스트 반전을 통해 StableDiffusion의 새로운 개념 가르치기
nav_order: 05+00
...
**생성일:** 2022/12/09  
**최종편집일:** 2022/12/09  
**설명:** Learning new visual concepts with KerasCV's StableDiffusion implementation.
...
Next, we implement a `StableDiffusionFineTuner`, which is a subclass of [`keras.Model`](/api/models/model#model-class) that overrides `train_step` to train the token embeddings of our text encoder. This is the core of the Textual Inversion algorithm.
...
To monitor training, we can produce a [`keras.callbacks.Callback`](/api/callbacks/base_callback#callback-class) to produce a few images every epoch using our custom token.
