...
이는 우수한 성능에 필수적입니다. 
우리는 [람다 레이어]({{ site.baseurl }}/api/layers/core_layers/lambda/)를 사용하여, 
사인파 임베딩을 구현합니다.
...
* 문헌에서는 더 나은 전역적 코히어런스(global coherence)를 위해, 낮은 해상도에서 [어텐션 레이어]({{ site.baseurl }}/api/layers/attention_layers/multi_head_attention/)를 사용하는 것이 일반적입니다. 단순성을 위해 생략했습니다.
...
픽셀별 [평균 제곱 오차]({{ site.baseurl }}/api/losses/regression_losses/#mean_squared_error-function)를 이론적으로 사용해야 하지만, 
저는 대신 [평균 절대 오차]({{ site.baseurl }}/api/losses/regression_losses/#mean_absolute_error-function)를 사용하는 것을 권장합니다. (이 구현과 유사)
...
* **이미지 보강**: 이 예에서는 이미지 증강을 사용하지 않았지만, 
  ...
  또한, [누출 보강]({{ site.baseurl }}/examples/generative/gan_ada/#invertible-data-augmentation)을 사용하지 않도록 주의해야 합니다. 
...
* **정규화**: 모델을 확장할 때 가끔씩 발산된 트레이닝을 마주쳤는데, 
  ...
  문헌에서는 [GroupNormalization](https://www.tensorflow.org/addons/api_docs/python/tfa/layers/GroupNormalization)(예를 들어 8개 그룹) 또는 [LayerNormalization]({{ site.baseurl }}/api/layers/normalization_layers/layer_normalization/)을 네트워크에서 사용하는 것이 일반적이지만, 
  저는 [BatchNormalization]({{ site.baseurl }}/api/layers/normalization_layers/batch_normalization/)을 사용하기로 했습니다. 
...
* **어텐션**: 앞서 언급했듯이, 더 나은 전역적 코히어런스를 위해 저해상도에서, 
  [어텐션 레이어]({{ site.baseurl }}/api/layers/attention_layers/multi_head_attention/)를 사용하는 것이 문헌에서 일반적입니다. 
  단순화를 위해 생략했습니다.
* **업샘플링**: 네트워크에서 [이중선형 및 최근접 이웃 업샘플링]({{ site.baseurl }}/api/layers/reshaping_layers/up_sampling2d/)은 비슷한 성능을 보였지만, 
  [transposed 컨볼루션]({{ site.baseurl }}/api/layers/convolution_layers/convolution2d_transpose/)은 시도하지 않았습니다.

GAN에 대한 유사한 리스트는 [이 Keras 튜토리얼]({{ site.baseurl }}/examples/generative/gan_ada/#gan-tips-and-tricks)을 확인하세요.
