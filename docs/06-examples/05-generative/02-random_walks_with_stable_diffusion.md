---
layout: default
title: Stable Diffusion으로 잠재된 공간 걷기
nav_order: 02+00
...
# Stable Diffusion으로 잠재된 공간 걷기 (A walk through latent space with Stable Diffusion)
{: .no_toc }
...
아직 읽지 않았다면, [Stable Diffusion 튜토리얼]({{ site.baseurl }}/guides/keras_cv/generate_images_with_stable_diffusion/)을 읽어보세요.

시작하려면, KerasCV를 가져와서 튜토리얼 [Stable Diffusion으로 이미지 생성]({{ site.baseurl }}/guides/keras_cv/generate_images_with_stable_diffusion/)에서 설명한 최적화를 사용하여, 
...
![Dog to Fruit 5]({{ site.baseurl }}/img/examples/generative/random_walks_with_stable_diffusion/4ZCxZY4.gif)
...
확산 노이즈가 변하도록 허용하면서 보간할 수도 있습니다.
...
![Eiffel tower walk gif]({{ site.baseurl }}/img/examples/generative/random_walks_with_stable_diffusion/9MMYtal.gif)
...
단일 프롬프트에 대한 확산 노이즈 공간을 통한 원형 워크(circular walk)
---------------------------------------------------------------------
{: #a-circular-walk-through-the-diffusion-noise-space-for-a-single-prompt}
<!-- A circular walk through the diffusion noise space for a single prompt -->

마지막 실험은 하나의 프롬프트에 집중하고, 
확산 모델이 그 프롬프트에서 생성할 수 있는 다양한 이미지를 탐색하는 것입니다. 
확산 프로세스를 시딩하는 데 사용되는 노이즈를 제어하여 이를 수행합니다.
...
텍스트 인코더의 잠재 매니폴드와 확산 모델의 노이즈 공간을 탐색하는 것은, 
