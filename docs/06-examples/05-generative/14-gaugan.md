...
**저자:** [Soumik Rakshit](https://github.com/soumik12345), [Sayak Paul](https://twitter.com/RisingSayak)  
**생성일:** 2021/12/26  
**최종편집일:** 2022/01/03  
**설명:** 조건부 이미지 생성을 위한 GauGAN 구현

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/generative/ipynb/gaugan.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/generative/gaugan.py){: .btn .btn-blue }
...
*   keras.io의 GAN 구현 예시들:
    * [데이터 효율적인 GAN]({{ site.baseurl }}/examples/generative/gan_ada) 
    * [CycleGAN]({{ site.baseurl }}/examples/generative/cyclegan) 
    * [조건부 GAN]({{ site.baseurl }}/examples/generative/conditional_gan)
...
GauGAN 모델을 학습하기 위해 [Facades 데이터셋](https://cmp.felk.cvut.cz/~tylecr1/facade/)을 사용할 것입니다. 
먼저 이를 다운로드해 보겠습니다.
...
print(f"세그멘테이션 맵 배치 크기: {sample_train_batch[0].shape}.")
print(f"이미지 배치 크기: {sample_train_batch[1].shape}.")
print(f"원-핫 인코딩된 레이블 맵 크기: {sample_train_batch[2].shape}.")

# 트레이닝 세트에서 몇 가지 샘플을 플롯합니다.
for segmentation_map, real_image in zip(sample_train_batch[0], sample_train_batch[1]):
    fig = plt.figure(figsize=(10, 10))
    fig.add_subplot(1, 2, 1).set_title("세그멘테이션 맵")
    plt.imshow((segmentation_map + 1) / 2)
    fig.add_subplot(1, 2, 2).set_title("실제 이미지")
...
마지막으로, 모든 것을 서브클래싱된 모델([`tf.keras.Model`]({{ site.baseurl }}/api/models/model#model-class)로부터) 안에 넣고, 
