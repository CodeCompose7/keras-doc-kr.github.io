...
**저자:** [Anshuman Mishra](https://github.com/shivance/)  
**생성일:** 2023/02/25  
**최종편집일:** 2023/02/25  
**설명:** Use pretrained models from KerasNLP for the Semantic Similarity Task.

[Colab에서 보기](https://colab.research.google.com/github/keras-team/keras-io/blob/master/examples/nlp/ipynb/semantic_similarity_with_keras_nlp.ipynb){: .btn .btn-blue }
[GitHub 소스](https://github.com/keras-team/keras-io/blob/master/examples/nlp/semantic_similarity_with_keras_nlp.py){: .btn .btn-blue }
...
Here's a utility function that splits the example into an `(x, y)` tuple that is suitable for `model.fit()`. By default, [`keras_nlp.models.BertClassifier`](/api/keras_nlp/models/bert/bert_classifier#bertclassifier-class) will tokenize and pack together raw strings using a `"[SEP]"` token during training. Therefore, this label splitting is all the data preparation that we need to perform.
...
We use the BERT model from KerasNLP to establish a baseline for our semantic similarity task. The [`keras_nlp.models.BertClassifier`](/api/keras_nlp/models/bert/bert_classifier#bertclassifier-class) class attaches a classification head to the BERT Backbone, mapping the backbone outputs to a logit output suitable for a classification task. This significantly reduces the need for custom code.
...
Just tweaking the learning rate alone was not enough to boost performance, which stayed right around 76%. Let's try again, but this time with [`keras.optimizers.AdamW`](/api/optimizers/adamw#adamw-class), and a learning rate schedule.
