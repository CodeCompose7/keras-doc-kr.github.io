...
# 저장 및 직렬화 커스터마이징 (Customizing Saving and Serialization)
{: .no_toc }
...
대부분의 사용자에게는 기본 [직렬화, 저장 및 내보내기 가이드]({{ site.baseurl }}/guides/serialization_and_saving)에서 
설명된 방법으로 충분할 것입니다.
...
class LayerWithCustomBuild(keras.layers.Layer):
    ...
    def build(self, input_shape, layer_init):
        ...
        # 따라서, `call()`을 처음 실행하기 전에 `layer_init` 인자로 수동으로 `build()`를 호출해야 합니다.
