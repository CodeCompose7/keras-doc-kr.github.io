---
layout: default
title: 커스텀 트레이닝 루프에서 하이퍼파라미터 튜닝
nav_order: 3
...
# 커스텀 트레이닝 루프에서 하이퍼파라미터 튜닝 (Tune hyperparameters in your custom training loop)
{: .no_toc }
...
**생성일:** 2019/10/28  
**최종편집일:** 2022/01/12  
**설명:** `HyperModel.fit()`을 사용하여 트레이닝 하이퍼파라미터(예: 배치 크기)를 튜닝합니다.
...
-   `hp` 객체: [`keras_tuner.HyperParameters`]({{ site.baseurl }}/api/keras_tuner/hyperparameters#hyperparameters-class)의 인스턴스
-   `HyperModel.build()`에 의해 빌드된 모델

기본 예시는 [KerasTuner 시작하기]({{ site.baseurl }}/guides/keras_tuner/getting_started/#tune-model-training)의 "모델 트레이닝 튜닝하기" 섹션에서 확인할 수 있습니다.
...
[처음부터 트레이닝 루프 작성하기]({{ site.baseurl }}/guides/writing_a_training_loop_from_scratch/) 가이드를 참조하십시오.
...
그럼, `MyHyperModel`로 `HyperModel` 클래스를 서브클래싱하겠습니다. 
...
objective 이름은 콜백의 `on_epoch_end()` 메서드에 전달된 `logs`에서 사용하는 키와 일치해야 합니다. 
...
[KerasTuner로 시작하기]({{ site.baseurl }}/guides/keras_tuner/getting_started/)를 참조하세요.
