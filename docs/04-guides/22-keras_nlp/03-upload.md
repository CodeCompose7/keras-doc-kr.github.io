...
**생성일:** 2024/04/29  
**최종편집일:** 2024/04/29  
**설명:** An introduction on how to upload a fine-tuned KerasNLP model to model hubs.
...
This guide walks you through how to upload your fine-tuned models to popular model hubs such as [Kaggle Models](https://www.kaggle.com/models) and [Hugging Face Hub](https://huggingface.co/models).
...
A [`keras_nlp.models.Task`]({{ site.baseurl }}/api/keras_nlp/base_classes/task#task-class), wraps a [`keras_nlp.models.Backbone`]({{ site.baseurl }}/api/keras_nlp/base_classes/backbone#backbone-class) and a [`keras_nlp.models.Preprocessor`]({{ site.baseurl }}/api/keras_nlp/base_classes/preprocessor#preprocessor-class) to create a model that can be directly used for training, fine-tuning, and prediction for a given text problem. In this section, we explain how to create a `Task`, fine-tune and upload it to a model hub.
...
If you want to build a Causal LM based on a base model, simply call [`keras_nlp.models.CausalLM.from_preset`]({{ site.baseurl }}/api/keras_nlp/base_classes/causal_lm#frompreset-method) and pass a built-in preset identifier.
...
```
100/100 ━━━━━━━━━━━━━━━━━━━━ 151s 1s/step - loss: 1.0198 - sparse\_categorical\_accuracy: 0.3271
```
...
You can also load the [`keras_nlp.models.Backbone`]({{ site.baseurl }}/api/keras_nlp/base_classes/backbone#backbone-class) and [`keras_nlp.models.Tokenizer`]({{ site.baseurl }}/api/keras_nlp/base_classes/tokenizer#tokenizer-class) objects from this preset directory. Note that these objects are equivalent to `causal_lm.backbone` and `causal_lm.preprocessor.tokenizer` above.
...
After saving a preset to a directory, this directory can be uploaded to a model hub such as Kaggle or Hugging Face directly from the KerasNLP library. To upload the model to Kaggle, the URI must start with `kaggle://` and to upload to Hugging Face, it should start with `hf://`.
...
To upload a model we can use `keras_nlp.upload_preset(uri, preset_dir)` API where `uri` has the format of `kaggle://<KAGGLE_USERNAME>/<MODEL>/Keras/<VARIATION>` for uploading to Kaggle and `preset_dir` is the directory that the model is saved in.
...
`keras_nlp.upload_preset(uri, preset_dir)` can be used to upload a model to Hugging Face if `uri` has the format of `kaggle://<HF_USERNAME>/<MODEL>`.
...
Uploading a classifier model is similar to Causal LM upload. To upload the fine-tuned model, first, the model should be saved to a local directory using `save_to_preset` API and then it can be uploaded via [`keras_nlp.upload_preset`]({{ site.baseurl }}/api/keras_nlp/base_classes/upload_preset#uploadpreset-function).
...
```
100/100 ━━━━━━━━━━━━━━━━━━━━ 7s 31ms/step - loss: 0.6975 - sparse\_categorical\_accuracy: 0.5164
