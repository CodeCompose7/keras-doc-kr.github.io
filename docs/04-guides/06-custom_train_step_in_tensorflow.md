---
layout: default
title: TensorFlow에서 `fit()`의 동작을 커스터마이즈하기
nav_order: 6
...
# TensorFlow에서 `fit()`의 동작을 커스터마이즈하기 (Customizing what happens in `fit()` with TensorFlow)
{: .no_toc }
...
- [`keras.Model`]({{ site.baseurl }}/api/models/model#model-class)을 서브클래스화하는, 
  새로운 클래스를 만듭니다.
