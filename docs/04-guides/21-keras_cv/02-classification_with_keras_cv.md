...
# KerasCV로 분류하기 (Classification with KerasCV)
{: .no_toc }
...
_task_ 는 (일반적으로 사전 트레이닝된) 백본 모델과 작업별 레이어로 구성된 [`keras.Model`]({{ site.baseurl }}/api/models/model#model-class)입니다. 
다음은 EfficientNetV2B0 백본과 함께 [`keras_cv.models.ImageClassifier`]({{ site.baseurl }}/api/keras_cv/models/tasks/image_classifier#imageclassifier-class)를 사용하는 예입니다.
...
이 경우 결정론적(deterministic) [`keras_cv.layers.Resizing`]({{ site.baseurl }}/api/keras_cv/layers/preprocessing/resizing#resizing-class)을 사용하여, 
...
* [KerasCV의 데이터 보강]({{ site.baseurl }}/guides/keras_cv/cut_mix_mix_up_and_rand_augment/)에 대해 자세히 알아보세요.
