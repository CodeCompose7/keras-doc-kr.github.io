...
# KerasCV로 객체 감지 (Object Detection with KerasCV)
{: .no_toc }
...
이 API에는 [`keras_cv.models.YOLOV8Detector`]({{ site.baseurl }}/api/keras_cv/models/tasks/yolo_v8_detector#yolov8detector-class)와 같은 완전히 사전 트레이닝된 객체 감지 모델이 포함됩니다.
...
간단히 `pad_to_aspect_ratio=True`를 [`keras_cv.layers.Resizing`]({{ site.baseurl }}/api/keras_cv/layers/preprocessing/resizing#resizing-class) 레이어에 전달하면 됩니다.
...
다른 [`keras.Model`]({{ site.baseurl }}/api/models/model#model-class)과 마찬가지로, 
...
다행히도, KerasCV는 광범위한 [데이터 보강 레이어]({{ site.baseurl }}/api/keras_cv/layers/preprocessing/) 라이브러리를 통해, 
...
[`keras.callbacks.ReduceLROnPlateau`]({{ site.baseurl }}/api/callbacks/reduce_lr_on_plateau/) 콜백을 사용하여 학습률을 줄입니다.
...
대단합니다! 
마지막으로 알아두어야 할 유용한 패턴은 [`keras.callbacks.Callback`]({{ site.baseurl }}/api/callbacks/base_callback#callback-class)에서 감지를 시각화하여 트레이닝을 모니터링하는 것입니다.
