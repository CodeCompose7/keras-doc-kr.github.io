...
![]({{ site.baseurl }}/img/guides/object_detection_keras_cv/8xSEbQD.png)
...
Since the inception of [_You Only Look Once_](https://arxiv.org/abs/1506.02640) (aka YOLO), object detection has primarily been solved using deep learning. Most deep learning architectures do this by cleverly framing the object detection problem as a combination of many small classification problems and many regression problems.
...
The highest level API in the KerasCV Object Detection API is the `keras_cv.models` API. This API includes fully pretrained object detection models, such as [`keras_cv.models.YOLOV8Detector`]({{ site.baseurl }}/api/keras_cv/models/tasks/yolo_v8_detector#yolov8detector-class).
...
This argument describes _exactly_ what format the values in the `"boxes"` field of the label dictionary take in your pipeline. For example, a box in `xywh` format with its top left corner at the coordinates (100, 100) with a width of 55 and a height of 70 would be represented by:
...
While this may seem simple, it is a critical piece of the KerasCV object detection API! Every component that processes bounding boxes requires a `bounding_box_format` argument. You can read more about KerasCV bounding box formats [in the API docs]({{ site.baseurl }}/api/keras_cv/bounding_box/formats/).
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_8_0.png)

![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_8_1.png)
...
If the resize operation distorts the input's aspect ratio, the model will perform signficantly poorer. For the pretrained `"yolo_v8_m_pascalvoc"` preset we are using, the final `MeanAveragePrecision` on the `pascalvoc/2012` evaluation set drops to `0.15` from `0.38` when using a naive resizing operation.
...
KerasCV makes resizing properly easy; simply pass `pad_to_aspect_ratio=True` to a [`keras_cv.layers.Resizing`]({{ site.baseurl }}/api/keras_cv/layers/preprocessing/resizing#resizing-class) layer.
...
`keras_cv.visualization.plot_bounding_box_gallery()` supports a `class_mapping` parameter to highlight what class each box was assigned to. Let's assemble a class mapping now.
...
Just like any other [`keras.Model`]({{ site.baseurl }}/api/models/model#model-class) you can predict bounding boxes using the `model.predict()` API.
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_16_2.png)
...
Non-max suppression is a highly configurable algorithm, and in most cases you will want to customize the settings of your model's non-max suppression operation. This can be done by overriding to the `prediction_decoder` argument.

To show this concept off, let's temporarily disable non-max suppression on our YOLOV8Detector. This can be done by writing to the `prediction_decoder` attribute.
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_18_2.png)
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_20_2.png)
...
`bounding_boxes['boxes']` contains the coordinates of your bounding box in a KerasCV supported `bounding_box_format`. KerasCV requires a `bounding_box_format` argument in all components that process bounding boxes. This is done to maximize your ability to plug and play individual components into their object detection pipelines, as well as to make code self-documenting across object detection pipelines.
...
This clearly yields bounding boxes in the format `xywh`. You can read more about KerasCV bounding box formats [in the API docs]({{ site.baseurl }}/api/keras_cv/bounding_box/formats/).
...
To construct a ragged dataset in a [`tf.data`](https://www.tensorflow.org/api_docs/python/tf/data) pipeline, you can use the `ragged_batch()` method.
...
Let's make sure our dataset is following the format KerasCV expects. By using the `visualize_dataset()` function, you can visually verify that your data is in the format that KerasCV expects. If the bounding boxes are not visible or are visible in the wrong locations that is a sign that your data is mis-formatted.
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_28_0.png)
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_30_0.png)
...
Luckily, KerasCV natively supports bounding box augmentation with its extensive library of [data augmentation layers]({{ site.baseurl }}/api/keras_cv/layers/preprocessing/). The code below loads the Pascal VOC dataset, and performs on-the-fly, bounding-box-friendly data augmentation inside a [`tf.data`](https://www.tensorflow.org/api_docs/python/tf/data) pipeline.
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_32_0.png)
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_36_0.png)
...
In this guide, we use a standard SGD optimizer and rely on the [`keras.callbacks.ReduceLROnPlateau`]({{ site.baseurl }}/api/callbacks/reduce_lr_on_plateau/) callback to reduce the learning rate.
...
To achieve the best results on your dataset, you'll likely want to hand craft a `PiecewiseConstantDecay` learning rate schedule. While `PiecewiseConstantDecay` schedules tend to perform better, they don't translate between problems.

### Loss functions
{: #loss-functions}
<!-- ### Loss functions -->

You may not be familiar with the `"ciou"` loss. While not common in other models, this loss is sometimes used in the object detection world.
...
That is all it takes to construct a KerasCV YOLOv8. The YOLOv8 accepts tuples of dense image Tensors and bounding box dictionaries to `fit()` and `train_on_batch()`
...
All that is left to do is train our model. KerasCV object detection models follow the standard Keras workflow, leveraging `compile()` and `fit()`.
...
KerasCV makes object detection inference simple. `model.predict(images)` returns a tensor of bounding boxes. By default, `YOLOV8Detector.predict()` will perform a non max suppression operation for you.
...
Awesome! One final helpful pattern to be aware of is to visualize detections in a [`keras.callbacks.Callback`]({{ site.baseurl }}/api/callbacks/base_callback#callback-class) to monitor training :
...
![png]({{ site.baseurl }}/img/guides/object_detection_keras_cv/object_detection_keras_cv_64_2.png)