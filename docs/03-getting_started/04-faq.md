...
* [JAX를 사용한 다중 GPU 분산 트레이닝]({% link docs/04-guides/15-distributed_training_with_jax.md %})
* [TensorFlow를 사용한 다중 GPU 분산 트레이닝]({% link docs/04-guides/16-distributed_training_with_tensorflow.md %})
* [PyTorch를 사용한 다중 GPU 분산 트레이닝]({% link docs/04-guides/17-distributed_training_with_torch.md %})

모델 병렬 처리의 경우, Keras는 자체 분산 API를 가지고 있으며, 현재는 JAX 백엔드에서만 지원합니다. 
[`LayoutMap` API에 대한 문서]({% link docs/05-api/12-distribution.md %})를 참조하세요.
...
* TPU 활용(utilized)을 계속 유지할 수 있을 만큼, 빠르게 데이터를 읽을 수 있는지 확인하세요.
* TPU 활용(utilized)을 계속 유지하기 위해, 그래프 실행당 여러 단계의 경사 하강을 실행하는 것을 고려하세요. 
...
예를 들어, MacBook Pro에서는 `/Users/fchollet/.keras/`입니다.
...
마찬가지로, [`get_file()`]({% link docs/05-api/14-utils.md %}#get_file)로 다운로드한 것과 같은 캐시된 데이터 세트 파일은 기본적으로 `$HOME/.keras/datasets/`에 저장되고, 
...
[KerasTuner]({% link docs/07-keras_tuner/00-keras_tuner.md %}) 사용을 권장합니다.
...
1. Keras 자체. (예: `keras.random` 연산 또는 `keras.layers`의 무작위 레이어)
...
(프로그램 자체 내에서가 아님) 
이는 Python 3.2.3 이상에서 특정 해시 기반 작업(예: 집합 또는 사전의 항목 순서, [Python 문서](https://docs.python.org/3.7/using/cmdline.html#envvar-PYTHONHASHSEED) 참조)에 대해, 
재현 가능한 동작을 하기 위해 필요합니다.

CUDA 런타임을 결정론적으로 만들려면 TensorFlow 백엔드를 사용하는 경우, 
...
아래 코드와 파일 확장자 `.weights.h5`를 사용하여, HDF5에서 저장할 수 있습니다.
...
# 새로운 모델
model = Sequential()
model.add(Dense(2, input_dim=3, name='dense_1'))  # 로드됩니다
model.add(Dense(10, name='new_dense'))  # 로드되지 않습니다
...
**4) 저장된 모델에서 커스텀 레이어(또는 다른 커스텀 객체) 처리**
...
또는, [커스텀 개체 범위]({% link docs/05-api/14-utils.md %}#customobjectscope)를 사용할 수 있습니다.
...
이는 Keras의 종속성이며 기본적으로 설치해야 합니다. 
...
그렇지 않으면, [자세한 설치 지침은 여기](http://docs.h5py.org/en/latest/build.html)에서 확인할 수 있습니다.
...
* **배치 (Batch)**: _N_ 개의 샘플 집합입니다. 
  ...
  **배치**는 일반적으로 단일 입력보다 입력 데이터의 분포를 더 잘 근사합니다. 
  ...
  추론(평가/예측)의 경우, 메모리를 초과하지 않고도 가능한 한 큰 배치 크기를 선택하는 것이 좋습니다.
...
* **에포크 (Epoch)**: 일반적으로 "전체 데이터 세트에 대한 한 번의 패스"로 정의되는 임의의 컷오프로, 
  ...
  Keras 내에서는 **에포크**가 끝날 때마다 실행되도록 특별히 설계된 [콜백]({% link docs/05-api/03-callbacks.md %})을 추가할 수 있습니다. 
...
드롭아웃 및 L1/L2 가중치 정규화와 같은 정규화 메커니즘은 테스트 시간에 꺼집니다. 
이는 트레이닝 시간 손실에는 반영되지만, 테스트 시간 손실에는 반영되지 않습니다.
...
다음에 `Model.fit()`를 호출할 때 로드하는 [`keras.callbacks.BackupAndRestore`]({% link docs/05-api/03-callbacks/03-backup_and_restore.md %}#backupandrestore-class) 콜백을 사용해야 합니다.
...
try:
  ...
  print('***방해 처리***')
  # 이것은 중단된 에포크로부터 계속됩니다.
  model.fit(x, y, epochs=20, steps_per_epoch=5,
            callbacks=[backup_callback])
```

자세한 내용은 [콜백 문서]({% link docs/05-api/03-callbacks.md %})에서 확인하세요.

* * *

### 검증 손실이 더 이상 감소하지 않을 때, 트레이닝을 중단하려면 어떻게 해야 합니까?
{: #how-can-i-interrupt-training-when-the-validation-loss-isnt-decreasing-anymore}
...
* 기본 모델을 인스턴스화하고 사전 트레이닝된 가중치를 로드합니다.
* 해당 기본 모델을 동결합니다.
...
{: #whats-the-difference-between-the-training-argument-in-call-and-the-trainable-attribute}
...
예를 들어, 트레이닝 모드에서 `Dropout` 레이어는 무작위 드롭아웃을 적용하고 출력을 재조정합니다. 
추론 모드에서는 동일한 레이어가 아무 작업도 수행하지 않습니다. 예:

```python
y = Dropout(0.5)(x, training=True)  # 트레이닝 시간 *및* 추론 시간에 드롭아웃을 적용합니다.
```
...
# 훈련 시간 *및* 추론 시간에 드롭아웃을 적용하고, 트레이닝 중에 스케일링 요소를 학습합니다.
...
```python
# *동결된* 스케일링 요소로 트레이닝 시간 *및* 추론 시간에 드롭아웃을 적용합니다.
...
**_`BatchNormalization` 레이어의 특수 사례_**
...
다른 모든 레이어의 경우, 
가중치 트레이닝 가능성과 "추론 vs 트레이닝 모드"는 독립적으로 유지됩니다.
...
검증 분할을 추출하기 전에 데이터가 셔플되지 않으므로, 
검증은 문자 그대로 전달한 입력의 샘플의 _마지막_ x%에 불과합니다.
...
### `fit()`에서, 트레이닝 중에 데이터가 셔플되나요?
{: #in-fit-is-the-data-shuffled-during-training}
<!-- ### In `fit()`, is the data shuffled during training? -->

데이터를 NumPy 배열로 전달하고, 
`model.fit()`의 `shuffle` 인수가 `True`(기본값)로 설정된 경우, 
학습 데이터는 각 에포크에서 전역적으로 무작위로 셔플됩니다.
...
데이터세트는 로컬로 셔플됩니다. (버퍼링 셔플링)

[`tf.data.Dataset`](https://www.tensorflow.org/api_docs/python/tf/data/Dataset) 객체를 사용하는 경우, 
버퍼 크기를 제어할 수 있도록, 사전에 데이터를 셔플링하는 것이 좋습니다.
(예: `dataset = dataset.shuffle(buffer_size)`를 호출)
...
{: #whats-the-recommended-way-to-monitor-my-metrics-when-training-with-fit}
...
이 기능은 트레이닝 및 검증 메트릭의 보기 좋은 그래프를 표시하며, 
트레이닝 중에 정기적으로 업데이트되며 브라우저에서 액세스할 수 있습니다.

[`TensorBoard` 콜백]({% link docs/05-api/03-callbacks/04-tensorboard.md %})을 통해, 
`fit()`와 함께 TensorBoard를 사용할 수 있습니다.

* * *

### `fit()`의 기능을 커스터마이즈해야 하는 경우 어떻게 해야 하나요?
...
이 경우 [`keras.Sequential`]({% link docs/05-api/01-models/02-sequential.md %}#sequential-class)을 서브클래싱하고, 
[`keras.Model`]({% link docs/05-api/01-models/01-model.md %}#model-class) 대신 `train_step`을 재정의합니다.

다음 가이드를 참조하세요.

* [JAX에서 커스텀 트레이닝 단계 작성]({% link docs/04-guides/05-custom_train_step_in_jax.md %})
* [TensorFlow에서 커스텀 트레이닝 단계 작성]({% link docs/04-guides/06-custom_train_step_in_tensorflow.md %})
* [PyTorch에서 커스텀 트레이닝 단계 작성]({% link docs/04-guides/07-custom_train_step_in_torch.md %})
...
* [JAX에서 커스텀 트레이닝 루프 작성]({% link docs/04-guides/08-writing_a_custom_training_loop_in_jax.md %})
* [TensorFlow에서 커스텀 트레이닝 루프 작성]({% link docs/04-guides/09-writing_a_custom_training_loop_in_tensorflow.md %})
* [PyTorch에서 커스텀 트레이닝 루프 작성]({% link docs/04-guides/10-writing_a_custom_training_loop_in_torch.md %})

* * *

### `Model` 메서드 `predict()`와 `__call__()`의 차이점은 무엇입니까?
{: #whats-the-difference-between-model-methods-predict-and-__call__}
...
> "`x`에서 모델을 실행하고 출력 `y`를 검색합니다."라는 의미입니다. 
> 하지만 정확히 같은 것은 아닙니다.
>
...
> 한편, `model(x)`는 메모리 내에서 발생하며 확장되지 않습니다. 
> 반면에 `predict()`는 미분할 수 없습니다. 
...
당연히, `call`을 오버라이드하는 `Model`의 하위 클래스인 모델에서는 불가능합니다.

다음은 또 다른 예입니다. 특정 명명된 레이어의 출력을 반환하는 `Model`을 인스턴스화합니다.
...
[`keras.applications`에서 사용 가능한 모델]({% link docs/05-api/10-applications.md %}) 또는 
[KerasCV]({% link docs/08-keras_cv/00-keras_cv.md %}) 및 
[KerasNLP]({% link docs/09-keras_nlp/00-keras_nlp.md %})에서 사용 가능한 모델을 활용할 수 있습니다.
...
* `x1`과 `x2`가 연속적인 샘플 배치인 경우, `x2[i]`는 모든 `i`에 대해 `x1[i]`의 후속 시퀀스입니다.
